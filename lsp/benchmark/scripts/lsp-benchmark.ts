...
function getTimeoutForData(): number {
    if (!benchmarkData?.date) return NEW_TIMEOUT_MS;
...
const timeoutLinePlugin = {
    ...
    afterDraw(chart: any, _args: any, options: any) {
        ...
        ctx.textAlign = 'right';
        ctx.fillText(
            options.label || `${options.value} ms timeout`,
 ...
 */
function createLatencyDistributionChart(): void {
    if (!benchmarkData) return;
    const canvas = document.getElementById('latencyDistributionChart') as HTMLCanvasElement | null;
    const ctx = canvas?.getContext('2d');
    if (!ctx) return;
    ...
    const meanValues = checkers.map(c =>
        results
    ...
    const validP95 = p95Medians.filter((v): v is number => v != null);
...
function populateSummary(): void {
    ...
    if (el1) el1.textContent = String(testedPackages);

    const el2 = document.getElementById('summaryCheckers');
    if (el2) el2.textContent = String(checkers.length);

    const el3 = document.getElementById('summaryRuns');
    if (el3) el3.textContent = String(benchmarkData.runs_per_package || '-');
}
 ...
 */
function categorizeFailure(metrics: CheckerMetrics | undefined, packageError: string | null): string {
    ...
    if (metrics.ok_pct === 0) return 'All requests timed out';
    ...
    tbody.innerHTML = failures.map(f => `
        <tr>
            ...
            <td>${f.checker === 'All'
                ...
                : `<span class="checker-badge ${f.checker.toLowerCase()}">${f.checker}</span>`
            }</td>
