...
// Chart colors - improved for better visibility
const COLORS: Record<string, string> = {
    ranking: 'rgb(255, 99, 132)',
    paramStubs: 'rgb(59, 130, 246)',      // Blue - primary metric
    returnStubs: 'rgb(16, 185, 129)',     // Green
    param: 'rgb(139, 92, 246)',           // Purple
    return: 'rgb(249, 115, 22)',          // Orange
    pyright: 'rgb(236, 72, 153)'          // Pink
};
...
interface HistoricalEntry {
    ...
    paramCoverageStubs: number;
    returnCoverageStubs: number;
    pyrightCoverage: number;
}
...
type HistoricalDataMap = Record<string, HistoricalEntry[]>;
    ...
    try {
        ...
        const loadPromises = dates.map(async (date) => {
            try {
                ...
                if (response.ok) {
                    const data: Record<string, PackageDetails> = await response.json();
                    return { date, data };
                }
            } catch (e) {
        ...
        const validResults = results.filter((r): r is { date: string; data: Record<string, PackageDetails> } => r !== null);
...
function getCurrentRank(packageName: string): number {
...
function renderCharts(): void {
    const container = document.getElementById('charts-container')!;
    const filter = (document.getElementById('package-filter') as HTMLInputElement).value.toLowerCase();
    const limit = (document.getElementById('limit-select') as HTMLSelectElement).value;
    ...
    for (const packageName of packages) {
        ...
        // Create chart
        const ctx = (card.querySelector('canvas') as HTMLCanvasElement).getContext('2d')!;
...
function createChartData(data: HistoricalEntry[], primaryMetric: string): any {
    const labels = data.map(d => d.date);

    // Helper to determine if this metric is primary (shown with emphasis)
    const isPrimary = (metric: string) => metric === primaryMetric;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('package-filter')!.addEventListener('input',
